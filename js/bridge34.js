(function(){
    ...
    */
	...
	function tmpEngine(str,data,debug){
		...
		function eachStopFail(str) {
            str=str.replace(/<bg:continue \/>/gm,"@%continue;%@");
            str=str.replace(/<bg:break \/>/gm,"@%break;%@");
            return str;
        }

        /**
		 * <bg:log>bg{bgScope.name}</bg:log>
         * @param str
         * @returns {string|XML|void|*}
         */
        function logFail(str) {
            str=str.replace(/<bg:log>@%=/gm,"@%console.log(");
            str=str.replace(/%@<\/bg:log>/,");%@");
            return str;
        }
        ...
        function setTagToJs(str){
            var arr=str.split("<bg:set");
            var len=arr.length;
            var varStr="";
            var varArr=[];
            var newStr=arr[0];
		...
		<ul>
			...
			<bg:each items="bg{bgScope.list}" item="item" status="status" step="1">
				<li>
					...
					last:bg{status.last};
					step:bg{status.step}
					<bg:each items="bg{item.job}" item="job" status="jobStatus">
	 ...
	 * <script id="jsonScript" type="text/template">
		<ul>
			<bg:each items="bg{bgScope}" item="item" status="status" type="json">
 ...
 * <body>
		<div id="box">
			<h1>bg{bgScope.name}</h1>	
		</div>
	</body>
 * 
 * @param {Object} data
 * @param {Object} debug
 * var obj = bg("#box").replace({name: "fsdfasfsdasdf"});
	console.log(obj.prop("outerHTML"));
 */
OtherBridge.prototype.replace=function(data,debug){
	var curObject=$(this.param);
	//var s=tmpEngine(curObject.html(),data);
	var s=tmpEngine(curObject.prop("outerHTML"),data);
	if(debug){
		console.warn("模板引擎tmp解析后的:");
		console.log(s);
	}
	var html=htmlEngine(s,data,debug);
	curObject.replaceWith(html);
	return $(this.param);
};
/**
 * <div id="form">
		<input name="name" type="text" value="gys" />
		<input name="age" type="text" value="26" />
		...
		<input name="job" type="checkbox"  checked="checked" value="乒乓球"/>
		<input name="job" type="checkbox" checked="checked" value="足球"/>
		<textarea name="dex">我是描述文件</textarea>
		<input name="hidden" type="hidden"  value="我是隐藏的....."/>
		<button onclick="getParam()">获取数据</button>
	</div>
 * 
 * 获取需要提交的数去
 * @b {Boolean} true返回json,false返回字符串默认返回json
 * bg("#form").getForm(true);
 */
OtherBridge.prototype.getForm=function(b){
	var res={};
	$("[name]",$(this.param)).each(function(){
		var obj=$(this);
		var tag=obj[0].tagName.toLocaleLowerCase();
		var type=obj.attr("type");
		var name=obj.attr("name");
		if(tag=="input"){
			if(type==undefined||!type||type=="text"||type=="hidden"||type=="file"||type=="password"||type=="number"){
				res[name]=obj.val();
			}else if(type=="radio"&&obj.is(":checked")){
				res[name]=obj.val();
			}else if(type=="checkbox"&&obj.is(":checked")){
				res[name]?res[name].push(obj.val()):res[name]=[obj.val()];
			}
		}else if(tag=="select"||tag=="textarea"){
			res[name]=obj.val();
		}
        var trim=obj.attr("bg-trim");
		if(trim!=undefined){//去空
